...
- **対話型 UI** — 番号選択と Yes/No で全設定が完結（最終確認のみ `YES` 入力が必要）
- **日本語完全対応** — ロケール・キーマップ・IME・日本語フォントを一括設定
- **豊富なデスクトップ環境** — KDE・GNOME・Xfce・Budgie・COSMIC・Sway・Hyprland・Niri の 9 種類
...
設定完了後にサマリーを表示し、大文字で `YES` と入力してからインストールを実行します。
...
| GNOME | シンプル DE | 初心者〜 |
| Xfce | 軽量 DE | 初心者〜 |
| Budgie | エレガント DE（Wayland 専用） | 初心者〜 |
| COSMIC | 次世代 DE（Rust 製・Wayland） | 中級〜 |
| Sway | タイリング WM（Wayland） | 上級者向け |
| Hyprland | タイリング WM（Wayland） | 上級者向け |
| Niri | スクロール型タイリング WM（Wayland） | 上級者向け |
| なし | CLI のみ | 上級者向け |
...
> **BIOS 環境のパーティションについて**  
> BIOS（レガシー）環境でも、本スクリプトは GPT パーティションテーブルを使用します（GRUB は BIOS+GPT に対応）。MBR/DOS 形式を使いたい場合は「手動（fdisk）」を選択してください。
...
- インストール実行の最終確認では `YES`（大文字）の入力が必要です。`y` や `yes` では実行されません。
- **パスワードに `|`（パイプ文字）は使用できません。** 入力時にエラーが表示されます。
