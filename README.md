...
- SSD/HDDを自動判定し、zram（メモリ圧縮swap）を使った物理swapなし構成を推奨
- パーティション構成、ファイルシステム、ブートローダー（systemd-boot / GRUB）選択
- パーティション初期化時に `wipefs` で旧シグネチャを消去し、フォーマット失敗を防止
...
- GPU自動検出（NVIDIA/AMD/Intel）とドライバの自動インストール・カーネルパラメータ設定
  - NVIDIAはGPU世代を判定し、Pascal以前のレガシーGPUにはAURのレガシードライバを案内
- 日本語環境（ロケール・フォント・IME）のワンステップ設定
- WiFi、ミラーサーバー、ファイアウォール、印刷機能などの追加設定
- systemd-journald のログサイズ制限、`paccache.timer` による自動キャッシュクリーンなど、インストール後の運用を見据えたメンテナンス設定
...
- AURパッケージ（yayなど）の導入は本スクリプトの対象外です。必要な場合はインストール後に手動でセットアップしてください。
- 本スクリプトは自己責任でご利用ください。
