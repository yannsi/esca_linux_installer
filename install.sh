...
do_format_and_mount() {
  ...
  _mount_root() {
    ...
    case "${CONFIG[filesystem]:-ext4}" in
      ext4)
        run_cmd "旧署名の消去"              wipefs -a -f "$part"
  ...
  if [[ "$boot_mode" == "uefi" ]]; then
    ...
    run_cmd "EFI 旧署名の消去"          wipefs -a -f "$efi_part"
    run_cmd "EFI フォーマット (FAT32)"  mkfs.fat -F32 "$efi_part"
    if [[ -n "$swap_part" ]]; then
      run_cmd "swap 旧署名の消去" wipefs -a -f "$swap_part"
...
do_pacstrap() {
  ...
  for attempt in 1 2; do
    echo ""
    print_step "pacstrap 実行中 (試行 $attempt/2)"
    if pacstrap /mnt "${pkgs[@]}" 2>&1 | tee -a /tmp/myarchinstall.log; then
      print_ok "pacstrap 完了"
      break
    fi
    print_err "pacstrap 失敗（試行 $attempt/2）"
