...
_fetch_cached() {
  ...
  _valid() {
    ...
    if [[ -n "$magic" ]]; then
      ...
      # EscaSymbols.otf はグリフ1個だけの約 1.2KB しかないため、
      # 「1KB 未満は不正」のような閾値を入れると正当なフォントを弾く。
...
# Esca 専用グリフ（アンコウの誘引突起）を収めたフォントを配置する。
#
# 【重要】割り当て先は U+100000（Supplementary PUA-B / 第16面）。
# Nerd Fonts v3 は U+E000-F8FF に加えて U+F0001-F1AF0（Material Design Icons）も
# 使っているため、第15面までの私用領域は衝突の危険がある。第16面は未使用。
#
# 【重要】フォントを置くだけでは端末に出ない。
# 等幅フォント（JetBrainsMono など）にこの符号位置は無いので、fontconfig の
# フォールバックに Esca Symbols を追加して初めて拾われる。
# さらに fc-cache を走らせないと、初回ログインまで反映されない。
_install_esca_font() {
  local src="" home_dir
  home_dir=$(_host_home)
  ...
  if [[ -n "$home_dir" ]]; then
    local c
    for c in "EscaSymbols.otf" "fonts/EscaSymbols.otf" ".local/share/fonts/EscaSymbols.otf"; do
      [[ -f "${home_dir}/${c}" ]] && { src="${home_dir}/${c}"; break; }
    done
  fi

  # 引き継ぎ元に無ければ GitHub から直接取得する（壁紙と同じ方針）。
  # EscaSymbols.otf は CFF アウトライン（fontTools 生成）なので先頭は "OTTO"。
  if [[ -z "$src" ]]; then
    src=$(_fetch_cached "/tmp/esca-symbols.otf" \
      "https://raw.githubusercontent.com/${ESCA_DOTFILES_REPO#https://github.com/}/${ESCA_DOTFILES_BRANCH}/EscaSymbols.otf" \
      "OTTO")
  fi

  if [[ -z "$src" ]]; then
    print_warn "Esca グリフのフォントを取得できませんでした（プロンプトのアイコンは既定のままになります）"
    return 0
  fi

  run_cmd "Esca グリフのフォントを配置" bash -c "
    mkdir -p /mnt/usr/share/fonts/esca
    cp '$src' /mnt/usr/share/fonts/esca/EscaSymbols.otf
    chmod 644 /mnt/usr/share/fonts/esca/EscaSymbols.otf
  "

  run_cmd "Esca グリフを fontconfig のフォールバックに追加" bash -c "
    mkdir -p /mnt/etc/fonts/conf.d
    cat > /mnt/etc/fonts/conf.d/62-esca-symbols.conf << 'FCEOF'
<?xml version=\"1.0\"?>
<!DOCTYPE fontconfig SYSTEM \"fonts.dtd\">
<fontconfig>
  <!-- Esca 専用グリフ (U+100000) を等幅・サンセリフのフォールバックに加える。
       binding=\"weak\" にしているのは、既存のフォント指定より優先させないため。
       これが無いと、この符号位置を持つフォントが見つからず豆腐(□)になる。 -->
  <match target=\"pattern\">
    <test name=\"family\"><string>monospace</string></test>
    <edit name=\"family\" mode=\"append\" binding=\"weak\"><string>Esca Symbols</string></edit>
  </match>
  <match target=\"pattern\">
...
FCEOF
  ...
  run_cmd_soft "フォントキャッシュ更新" arch-chroot /mnt fc-cache -f > /dev/null || true
  print_ok "Esca グリフ (U+100000) を利用できるようにしました"
}
...
_print_inheritable_configs() {
  local dir="$1"
  local found=() c
  for c in waybar hypr niri fuzzel alacritty mako cava mpv rofi swaylock ranger fcitx5 mozc; do
...
step_config_source() {
  ...
  echo -e "  ${GRAY}Waybar・Hyprland/Niri・alacritty・fcitx5 などの設定を"
  ...
  if [[ "${CONFIG[config_source]}" == "git" ]]; then
    ...
    if [[ -n "$git_dir" ]]; then
      ...
      # 【重要】このリポジトリは niri 向けの構成で、.config/hypr を含まない。
      # Hyprland を選んでいる場合は「Hyprland 本体の設定はパッケージ既定のまま」
      # になることを明示しておく（Waybar 等の周辺設定は引き継がれる）。
...
BUILD_ID=rolling
ANSI_COLOR=\"1;33\"
HOME_URL=\"https://github.com/esca-linux\"
LOGO=${OS_ID}
OSREL
  "
...
EOF"
  ...
  # Esca 専用グリフのフォント。
  # 【順序注意】fontconfig の優先度設定より後に呼ぶこと。
  # こちらも conf.d にファイルを置くため、先に走らせると番号順の意図が読みにくくなる。
  # デスクトップの有無に関わらず入れる（starship は全構成で導入している）。
  _install_esca_font
...
EOF
    ...
    hyprland)
      local pkgs=(hyprland hyprpaper hyprlock hypridle
                  ...
                  # streamlink: 引き継ぎ元 dotfiles の radiostation.sh が
                  # radiko など「直接 mpv に渡せないストリーム」を解決するのに使う。
                  # 本スクリプトが生成するフォールバック版 radio.sh は SomaFM の
                  # 直リンクのみなので mpv だけで足りるが、ホスト設定を引き継いだ
                  # ときに無いと局選択が無反応になるため WM 側で必ず入れる。
                  streamlink
                  xorg-xwayland
                  ...
                  brightnessctl
                  nautilus
                  mousepad imv zathura zathura-pdf-mupdf xarchiver
...
QtVersion=6
Version=1.0
Website=https://github.com/esca-linux
MainScript=Main.qml
...
write_first_login_guide() {
  ...
  local guide_tmp="/tmp/myarch_guide_$$.txt"
  {
    ...
    echo "  ・追加の壁紙集（zip）はこちらから入手できます:"
    echo "      https://github.com/yannsi/archinstall/blob/main/wallpaper.zip"
    echo "  ・展開して使う例:"
    echo "       cd ~/Downloads"
    echo "       unzip wallpaper.zip -d ~/Pictures/wallpapers"
    echo "  ・システム全体で使いたい場合は次の場所に置いてください:"
