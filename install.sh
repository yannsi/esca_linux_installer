...
ask_password() {
  ...
  while true; do
    ...
    if [[ ${#pw1} -lt 8 ]]; then
      ...
      if ! confirm "この短いパスワードで続けますか？"; then
        continue
      fi
    fi
...
step_partition_scheme() {
  ...
  local scheme
  if [[ "$is_ssd" == "yes" ]]; then
    print_warn "検出されたディスクは SSD/NVMe です。書き込み寿命保護のため、物理 swap なし ＋ zram 構成が推奨されます。"
    scheme=$(select_from_list "パーティション構成を選択:" \
      "自動（SSD推奨） - EFI 512M(起動用) + / のみ（物理 swap なし、メモリ圧縮 zram 推奨）" \
      "自動 - EFI 512M(起動用) + swap 4G(予備領域) + /" \
      "手動（fdisk を起動）")
  else
    scheme=$(select_from_list "パーティション構成を選択:" \
      "自動（推奨） - EFI 512M(起動用) + swap 4G(予備領域) + /" \
      "自動 - EFI 512M(起動用) + / のみ（物理 swap なし）" \
      "手動（fdisk を起動）")
  fi
...
do_pacstrap() {
  ...
  local pkgs=(base sudo linux linux-firmware networkmanager vim ttf-nerd-fonts-symbols woff2-font-awesome)
  ...
  _run_pacstrap_with_progress() {
    ...
    while kill -0 "$pid" 2>/dev/null; do
      ...
      local last_inst=""
      last_inst=$(grep -oE '^\([0-9]+/[0-9]+\)' /tmp/myarchinstall.log 2>/dev/null \
...
do_gpu_drivers() {
  ...
  if echo "$gpu_info" | grep -qi 'nvidia'; then
    ...
    # nvidia ドライバとヘッダーのインストール (LTSカーネル用等にも対応するため dkms にしておく)
    run_cmd "NVIDIA ドライバインストール" \
      arch-chroot /mnt pacman -S --noconfirm nvidia-dkms linux-headers nvidia-utils
