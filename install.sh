...
<fontconfig>
  <!-- Esca 専用グリフ (U+100000) をあらゆるフォントのフォールバックに加える。

       【重要】monospace / sans-serif だけに append してはいけない。
       fontconfig のフォールバックは「アプリが実際に要求した family 名」に
       対して働く。端末が \"monospace\" ではなく \"JetBrainsMono Nerd Font\" の
       ように実名を指定していると、monospace 向けの規則は一切通らず
       グリフが見つからずに豆腐(□)になる。
       実際 GNOME の既定端末は実名を指定するため、この方法では出なかった。

       そこで target=\"pattern\" かつ <test> を付けずに全パターンへ append する。
       これで family 指定の仕方によらず、末尾の候補として必ず参照される。
       binding=\"weak\" なので既存のフォント選択より優先されることはなく、
       他の文字の見た目には影響しない。 -->
