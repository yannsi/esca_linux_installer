...
ask_password() {
  ...
  while true; do
    echo -ne "  ${BOLD}${prompt}${RESET}: "
    read -rs pw1; echo
    # | はユーザーデータの区切り文字として使用しているため禁止
    if [[ "$pw1" == *"|"* ]]; then
      print_err "パスワードに '|' は使用できません。"
      continue
    fi
...
do_partition() {
  ...
  run_cmd "デバイスファイルの作成待機" udevadm settle
}
...
do_pacstrap() {
  ...
  case "${CONFIG[gpu_driver]}" in
    ...
    intel)
      ...
      case "${CONFIG[desktop]}" in
        gnome|kde|sway|hyprland|niri|cosmic|budgie) is_wayland_de="yes" ;;
      esac
...
do_aur_helper() {
  ...
  run_cmd_retry "AUR リポジトリのクローン (${repo_name})" \
  ...
  # makepkg はユーザー権限で実行
  run_cmd_retry "${helper} ビルド & インストール" \
    arch-chroot /mnt bash -c "
...
write_waybar_config() {
  ...
  local active_class
  case "$wm" in
    sway)     active_class="focused" ;;
    hyprland) active_class="active" ;;
    niri)     active_class="active" ;;
  esac
...
do_desktop() {
  ...
  case "${CONFIG[desktop]}" in
    ...
    budgie)
      local pkgs=(budgie budgie-control-center)
...
main() {
  ...
  if confirm "インストールを実行しますか？"; then
    ...
    local final_confirm
    echo -ne "  ${BOLD}${RED}実行する場合は大文字で YES と入力してください${RESET}: "
