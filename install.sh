...
declare -A CONFIG=(
  ...
  # ホスト環境クローン（このPCに入っているアプリ・設定を新環境へ複製する）
  [clone_host]="no"
  [clone_dotfiles]="no"
  [clone_pkg_file]="/tmp/myarchinstall-clone-pkgs.txt"
  [clone_aur_file]="/tmp/myarchinstall-clone-aur.txt"
)
...
step_system() {
  ...
  if confirm "Windows と共存（デュアルブート）しますか？"; then
    ...
    print_warn "重要: 自動パーティションは選択したディスク全体を消去します。"
    print_warn "Windows が同じディスク（${CONFIG[disk]}）にある場合は消去されるため、"
    print_warn "パーティション構成で「手動（fdisk）」を選び、空き領域に作成してください。"
    echo ""
...
step_desktop() {
  ...
  case "${CONFIG[desktop]}" in
    kde)
      ...
      ;;
    gnome|budgie)
      # greetd は非対応。greetd はセッションを「環境変数なしのコマンド」でしか
      # 起動できず、GNOME は XDG_SESSION_TYPE=wayland + dbus-run-session が必要、
      # Budgie は X11 セッション本体で X サーバ前提のため、どちらも素の
      # コマンド起動では立ち上がらない（tuigreet 公式もラッパー必須と明記）。
      dm=$(select_from_list "ディスプレイマネージャー (DM) を選択:" \
  ...
  # greetd × 非対応 DE の組み合わせが復活しないようにここでも弾く。
...
_clone_is_excluded() {
  case "$1" in
    # カーネルとマイクロコード（インストーラが選択・検出する）
    linux|linux-lts|linux-zen|linux-hardened|linux-rt|linux-rt-lts) return 0 ;;
    linux-headers|linux-*-headers|linux-firmware|linux-firmware-*)  return 0 ;;
    intel-ucode|amd-ucode)                                          return 0 ;;
    # GPU ドライバ（step_system で検出済み）
    nvidia|nvidia-*|lib32-nvidia-*|opencl-nvidia)                   return 0 ;;
    xf86-video-*|xf86-input-*)                                      return 0 ;;
    vulkan-radeon|vulkan-intel|vulkan-nouveau|lib32-vulkan-*)        return 0 ;;
    # ブートローダー（step_bootloader で選択済み）
    grub|efibootmgr|os-prober|refind)                               return 0 ;;
    # ディスプレイマネージャー（step_dm で DE ごとに選択済み・誤った組み合わせ防止）
    sddm|gdm|lightdm|lightdm-*|greetd|greetd-*|cosmic-greeter)      return 0 ;;
    # 仮想環境ゲストツール（新マシンの環境に合わせて入れる）
    virtualbox-guest-utils|qemu-guest-agent|open-vm-tools|spice-vdagent) return 0 ;;
    # Live ISO 由来・インストール作業用
    archiso|mkinitcpio-archiso|arch-install-scripts)                 return 0 ;;
    # メタパッケージ（pacstrap で導入済み）
    base|base-devel)                                                return 0 ;;
  esac
  return 1
}

step_host_clone() {
  print_step "ホスト環境のクローン"

  CONFIG[clone_host]="no"
  CONFIG[clone_dotfiles]="no"

  # Live ISO から起動している場合、見えるのは ISO 自身のパッケージなので意味がない
  if [[ -d /run/archiso ]]; then
    print_warn "Live ISO から実行中のため、クローン元になる環境がありません（スキップ）"
    return
  fi
  if [[ ! -d /var/lib/pacman/local ]]; then
    print_warn "ホスト側の pacman データベースが見つかりません（スキップ）"
    return
  fi

  # 明示的にインストールされたものだけを対象にする。
  # 依存で入っただけのパッケージは、本体を入れれば自動で付いてくるため不要。
  local all_official=() official=() aur=() p
  mapfile -t all_official < <(pacman -Qqen 2>/dev/null || true)
  mapfile -t aur          < <(pacman -Qqem 2>/dev/null || true)

  for p in "${all_official[@]}"; do
    _clone_is_excluded "$p" || official+=("$p")
  done

  local n_off="${#official[@]}" n_aur="${#aur[@]}"
  local n_excluded=$(( ${#all_official[@]} - n_off ))

  if [[ "$n_off" -eq 0 && "$n_aur" -eq 0 ]]; then
    print_warn "複製できるパッケージが見つかりませんでした（スキップ）"
    return
  fi
  if [[ "$n_off" -lt 50 ]]; then
    print_warn "検出されたアプリが ${n_off} 個と少なめです。最小構成の環境から実行していませんか？"
  fi

  echo ""
  echo -e "  このPCにインストール済みのアプリを、新しい環境にも入れられます。"
  echo -e "  ${BOLD}公式リポジトリ:${RESET} ${n_off} 個   ${BOLD}AUR:${RESET} ${n_aur} 個"
  echo -e "  ${GRAY}（カーネル・GPU ドライバなどハードウェア依存の ${n_excluded} 個は除外済み）${RESET}"
  echo ""

  if ! confirm_yes "このPCと同じアプリ構成にしますか？（推奨）"; then
    print_ok "ホスト環境のクローンはスキップ"
    return
  fi

  printf '%s\n' "${official[@]}" | sort -u > "${CONFIG[clone_pkg_file]}"
  if [[ "$n_aur" -gt 0 ]]; then
    printf '%s\n' "${aur[@]}" | sort -u > "${CONFIG[clone_aur_file]}"
  else
    : > "${CONFIG[clone_aur_file]}"
  fi
  CONFIG[clone_host]="yes"
  print_ok "アプリ一覧を控えました（公式 ${n_off} / AUR ${n_aur}）"

  # 設定ファイルのクローンは別途確認する。
  # /etc/skel 経由で「作成する全ユーザー」に配られるため、
  # 複数ユーザーを作る構成では意図しない共有になりうる。
  echo ""
  echo -e "  アプリの設定（~/.config など）もコピーできます。"
  echo -e "  ${GRAY}ブラウザ・メッセージアプリ・鍵/認証情報（.ssh, .gnupg）は除外されます。${RESET}"
  if [[ "${CONFIG[users_count]:-0}" -gt 1 ]]; then
    print_warn "ユーザーを複数作る設定です。設定は全ユーザーに同じものが配られます。"
  fi
  if confirm_yes "設定ファイルもコピーしますか？（推奨）"; then
    CONFIG[clone_dotfiles]="yes"
    print_ok "設定ファイルもコピーします"
  else
    print_ok "設定ファイルのコピーはスキップ"
  fi
}
...
show_summary() {
  ...
  if [[ "${CONFIG[clone_host]}" == "yes" ]]; then
    echo -e "  ${BOLD}環境クローン   :${RESET} 有効（アプリ $(wc -l < "${CONFIG[clone_pkg_file]}") 個 / AUR $(wc -l < "${CONFIG[clone_aur_file]}") 個・設定コピー: ${CONFIG[clone_dotfiles]}）"
  fi
...
do_chroot_config() {
  ...
  if [[ "${CONFIG[dualboot_windows]:-no}" == "yes" ]]; then
    ...
    run_cmd_soft "RTC UTC 設定" arch-chroot /mnt hwclock --systohc --utc
  fi
...
write_cosmic_font_config() {
  ...
  local tk_dir="/mnt/etc/skel/.config/cosmic/com.system76.CosmicTk/v1"
...
EOF"
  ...
  for app in CosmicEdit CosmicTerm; do
    local app_dir="/mnt/etc/skel/.config/cosmic/com.system76.${app}/v1"
...
write_libreoffice_cosmic_launchers() {
  ...
  run_cmd "COSMIC: LibreOffice ランチャーを XWayland 経由に上書き" bash -c '
    shopt -s nullglob
    dst=/mnt/etc/skel/.local/share/applications
    mkdir -p "$dst"
    found=0
...
write_screenshot_script() {
  ...
  cat > /mnt/etc/skel/.local/bin/screenshot.sh << 'SSEOF'
#!/bin/sh
...
SSEOF
  chmod +x /mnt/etc/skel/.local/bin/screenshot.sh
...
write_waybar_config() {
  ...
  if [[ -d "$host_waybar_dir" ]]; then
    ...
    run_cmd_soft "Waybar スクリプトに実行権を付与" \
      find "$dest_dir" -maxdepth 1 -name '*.sh' -exec chmod +x {} + || true
...
EOF
    ...
    hyprland)
      ...
      # ── 【要追跡 / 2026-07-15 時点】設定ファイル形式について ──
      # Hyprland 0.55 で設定言語が hyprlang(hyprland.conf) から Lua(hyprland.lua) へ移行した。
      # 旧 hyprlang は「0.55 から1〜2リリース」までサポートされ、その後に廃止される予定。
      # 新しい設定機能も hyprlang には追加されない。
      #   ・現状（Arch は 0.55.4）は hyprland.lua が無ければ hyprland.conf が読まれるため、
      #     以下の .conf 生成はそのまま動作する。
      #   ・0.56 / 0.57 あたりで読まれなくなる可能性があるため、その時点で Lua へ移行すること。
      #   ・移行時は hyprconf2lua 等の変換ツールが使える（約97%は自動変換できるとのこと）。
...
EOF
      ...
      if [[ "${CONFIG[jp_ime]:-none}" =~ ^fcitx5 ]]; then
        ...
        echo 'exec-once = fcitx5 -d -r' >> /mnt/etc/skel/.config/hypr/hyprland.conf
...
EOF
      ...
      # スクリーンショット（保存先を日本語のピクチャフォルダに統一）
      write_screenshot_script
      cat >> /mnt/etc/skel/.config/hypr/hyprland.conf << 'EOF'

# スクリーンショット（Print=範囲選択 / Shift+Print=全画面）。保存先: ~/ピクチャ/スクリーンショット
...
EOF
      ...
      if [ -f "$niri_cfg" ]; then
        ...
        print_ok "Niri: 追加バインド Mod+E=ファイル / Mod+Shift+B=ブラウザ / Mod+B=Waybar切替 / Print=スクショ"
      fi
      ...
      if [[ "${CONFIG[jp_ime]:-none}" =~ ^fcitx5 ]]; then
        ...
        mkdir -p /mnt/etc/skel/.config/autostart
        cat > /mnt/etc/skel/.config/autostart/org.fcitx.Fcitx5.desktop << 'FCITXEOF'
[Desktop Entry]
Type=Application
Name=Fcitx 5
Hidden=true
FCITXEOF
        ...
        echo 'spawn-at-startup "sh" "-c" "sleep 2 && fcitx5 -d -r"' >> /mnt/etc/skel/.config/niri/config.kdl
      elif [[ "${CONFIG[jp_ime]:-none}" =~ ^ibus ]]; then
...
# ============================================
# do_desktop の末尾と、DE なし構成での do_host_clone の両方から呼ばれる。
...
# ~/.config のうち複製しないもの。
# 除外理由は3種類 — プライバシー（鍵・認証情報・メッセージ履歴）、
# サイズ（ブラウザプロファイルは数百MBになる）、
# マシン固有の状態（dconf の DB、PulseAudio のデバイス設定など）。
_clone_dotfile_excluded() {
  case "$1" in
    # ブラウザ（プロファイル・Cookie・保存パスワードを含む）
    chromium|google-chrome|google-chrome-*|BraveSoftware|microsoft-edge*|vivaldi|opera*) return 0 ;;
    # メッセージ・通話アプリ（ログイン状態と会話履歴を含む）
    discord|Signal|Slack|teams*|Element|element-desktop)                                 return 0 ;;
    # 鍵・認証情報
    gnupg|sops|age|rclone|syncthing|keyrings|pass)                                       return 0 ;;
    # マシン固有の状態
    dconf|pulse|ibus|systemd)                                                            return 0 ;;
    # エディタ・IDE（拡張機能のキャッシュで肥大しやすい）
    Code|VSCodium|JetBrains)                                                             return 0 ;;
    # キャッシュ類
    *[Cc]ache*)                                                                          return 0 ;;
  esac
  return 1
...
# ホストPCの設定を /etc/skel へ複製する。
# /etc/skel に置くことで、作成される各ユーザーのホームに配られる。
_clone_copy_dotfiles() {
  local host_home
  host_home=$(_host_home)
  if [[ -z "$host_home" || ! -d "${host_home}/.config" ]]; then
    print_warn "ホストの設定ディレクトリが見つかりません（設定コピーをスキップ）"
    return 0
  fi

  mkdir -p /mnt/etc/skel/.config
  local copied=0 skipped=0 d name sz f

  # ~/.config 配下のディレクトリ
  for d in "${host_home}/.config"/*/; do
    [[ -d "$d" ]] || continue
    name="$(basename "$d")"
    if _clone_dotfile_excluded "$name"; then
      skipped=$(( skipped + 1 )); continue
    fi
    # 想定外に大きいものは除外する（キャッシュを抱えた設定ディレクトリ対策）
    sz=$(du -sm "$d" 2>/dev/null | cut -f1)
    if [[ "${sz:-0}" =~ ^[0-9]+$ ]] && [[ "$sz" -gt 20 ]]; then
      print_warn "${name} は ${sz}MB と大きいためスキップしました"
      skipped=$(( skipped + 1 )); continue
    fi
    cp -a "$d" /mnt/etc/skel/.config/ 2>/dev/null && copied=$(( copied + 1 ))
  done

  # ~/.config 直下の単体ファイル
  for f in "${host_home}/.config"/*; do
    [[ -f "$f" ]] || continue
    cp -a "$f" /mnt/etc/skel/.config/ 2>/dev/null || true
  done

  # シェル・エディタの設定（.ssh / .gnupg / .gitconfig は意図的に含めない）
  for f in .bashrc .bash_profile .zshrc .zprofile .profile .inputrc .vimrc .tmux.conf; do
    [[ -f "${host_home}/${f}" ]] && cp -a "${host_home}/${f}" /mnt/etc/skel/ 2>/dev/null || true
  done

  # 自作スクリプトとユーザーフォント
  if [[ -d "${host_home}/.local/bin" ]]; then
    mkdir -p /mnt/etc/skel/.local
    cp -a "${host_home}/.local/bin" /mnt/etc/skel/.local/ 2>/dev/null || true
    # ブラウザ経由でDLした際に実行権が落ちる事例があったため、ここで付け直す
    find /mnt/etc/skel/.local/bin -type f -exec chmod +x {} + 2>/dev/null || true
  fi
  if [[ -d "${host_home}/.local/share/fonts" ]]; then
    mkdir -p /mnt/etc/skel/.local/share
    cp -a "${host_home}/.local/share/fonts" /mnt/etc/skel/.local/share/ 2>/dev/null || true
  fi

  print_ok "設定をコピーしました（${copied} 件・除外 ${skipped} 件）"
}

do_host_clone() {
  [[ "${CONFIG[clone_host]}" != "yes" ]] && return

  # STEP_TOTAL に計上済みのため、スキップ時もステップ表示を先に行う
  print_step "ホスト環境のクローン"

  if [[ "${CONFIG[dry_run]}" == "yes" ]]; then
    print_warn "ドライランのためホスト環境のクローンをスキップします"
    return 0
  fi

  # ── 1) 設定ファイル ──
  if [[ "${CONFIG[clone_dotfiles]}" == "yes" ]]; then
    _clone_copy_dotfiles
  fi

  # ── 2) 公式リポジトリのパッケージ ──
  # ホストと新環境でリポジトリの中身が完全に一致するとは限らない（改名・削除）。
  # 存在しないパッケージが1つでも混ざると pacman は取引全体を中止するため、
  # 先に「新環境で入手可能なもの」と突き合わせてから渡す。
  local avail="/tmp/myarchinstall-clone-avail.txt"
  local want="/tmp/myarchinstall-clone-want.txt"
  local todo="/tmp/myarchinstall-clone-todo.txt"
  local gone="/tmp/myarchinstall-clone-gone.txt"

  if [[ -s "${CONFIG[clone_pkg_file]}" ]]; then
    arch-chroot /mnt pacman -Slq 2>/dev/null | sort -u > "$avail" || true
    sort -u "${CONFIG[clone_pkg_file]}" > "$want"
    if [[ -s "$avail" ]]; then
      comm -12 "$want" "$avail" > "$todo"
      comm -23 "$want" "$avail" > "$gone"
    else
      cp "$want" "$todo"; : > "$gone"
    fi

    if [[ -s "$gone" ]]; then
      print_warn "新環境のリポジトリに無いパッケージ $(wc -l < "$gone") 個を除外しました"
      echo "--- クローン対象外（リポジトリに存在しない） ---" >> "${CONFIG[log_file]}"
      cat "$gone" >> "${CONFIG[log_file]}"
    fi

    if [[ -s "$todo" ]]; then
      local pkgs=()
      mapfile -t pkgs < "$todo"
      print_ok "クローン対象: ${#pkgs[@]} 個"
      # まず一括で試す（依存解決が一度で済み、フックの実行回数も最小になる）
      if ! run_cmd_soft "ホストのアプリを一括インストール（${#pkgs[@]} 個・時間がかかります）" \
             arch-chroot /mnt pacman -S --noconfirm --needed "${pkgs[@]}"; then
        # 一括が失敗した場合、原因のパッケージを特定せずに済むよう分割して再試行する。
        # 25 個ずつなら、問題のある塊だけが落ちて残りは入る。
        print_warn "一括インストールに失敗したため、25個ずつに分けて再試行します"
        local i chunk
        for (( i = 0; i < ${#pkgs[@]}; i += 25 )); do
          chunk=("${pkgs[@]:i:25}")
          run_cmd_soft "インストール中 ($(( i + 1 ))-$(( i + ${#chunk[@]} )) / ${#pkgs[@]})" \
            arch-chroot /mnt pacman -S --noconfirm --needed "${chunk[@]}" || true
        done
      fi
    fi
  fi
  ...
  # ── 3) AUR パッケージ ──
  # yay が入っていない場合（AUR ヘルパーを選ばなかった等）はスキップする。
  if [[ -s "${CONFIG[clone_aur_file]}" ]]; then
    local helper_bin="/mnt/usr/bin/${CONFIG[aur_helper]}"
    if [[ ! -x "$helper_bin" ]]; then
      print_warn "AUR ヘルパーが無いため、AUR パッケージのクローンはスキップします"
      print_warn "後から実行できます: ${CONFIG[aur_helper]} -S --needed \$(cat 一覧)"
    else
      local first_user
      first_user=$(cut -d'|' -f1 <<< "$(head -n1 <<< "${CONFIG[users]}")")
      if [[ -z "$first_user" ]]; then
        print_warn "一般ユーザーが無いため AUR のクローンはスキップします"
      else
        # makepkg は root で実行できないため、do_aur_helper と同じ方式で
        # 一時的な NOPASSWD sudo を付与する（trap で必ず消える）
        local temp_sudoers="/etc/sudoers.d/${first_user}-aur-nopasswd"
        AUR_TEMP_SUDOERS="$temp_sudoers"
        run_cmd "一時的な sudo NOPASSWD 設定の追加" bash -c "
          echo '${first_user} ALL=(ALL:ALL) NOPASSWD: ALL' > /mnt${temp_sudoers}
        ...
        # AUR はビルドが失敗しうるので必ず1つずつ。1個の失敗で全体を止めない。
        local aurp
        while IFS= read -r aurp; do
          [[ -z "$aurp" ]] && continue
          run_cmd_soft "AUR: ${aurp}" \
            arch-chroot /mnt sudo -u "$first_user" -H \
              "${CONFIG[aur_helper]}" -S --noconfirm --needed \
              --answerdiff=None --answerclean=None --removemake "$aurp" || true
        done < "${CONFIG[clone_aur_file]}"
  ...
  # デスクトップ環境なしの構成では do_desktop の skel 同期が走らないため、
  # ここで各ユーザーのホームへ配る（DE ありの場合は do_desktop 側で実施）。
  if [[ "${CONFIG[clone_dotfiles]}" == "yes" && "${CONFIG[desktop]}" == "none" ]]; then
    _sync_skel_to_homes
  fi
}
...
write_first_login_guide() {
  ...
  {
    echo "========================================"
    echo " はじめにお読みください（myarchinstall）"
    echo "========================================"
    echo ""
    if [[ "${CONFIG[jp_ime]:-none}" =~ ^fcitx5 ]]; then
      echo "■ 日本語入力"
...
run_install() {
  ...
  [[ "${CONFIG[clone_host]}" == "yes"  ]] && STEP_TOTAL=$(( STEP_TOTAL + 1 ))
...
main() {
  ...
  while true; do
    ...
    case "$action" in
      ...
      "ホスト環境のクローン設定を変更する")         step_host_clone ;;
      "キャンセルして終了する")
        echo -e "\n  インストールを中断しました。"
        exit 0
        ;;
  ...
  if [[ "${CONFIG[dualboot_windows]}" == "yes" && "${CONFIG[partition_scheme]}" != "manual" ]]; then
    echo ""
    print_warn "Windows デュアルブートが有効ですが、自動パーティションが選択されています。"
    print_warn "自動パーティションはディスク ${CONFIG[disk]} 全体を消去します。"
    print_warn "Windows が同じディスクにある場合、Windows も完全に消去されます。"
    if ! confirm "Windows は別のディスクにインストールされていますか？"; then
      print_err "同じディスクに Windows がある場合は、設定の修正メニューから"
      print_err "パーティション構成を「手動（fdisk）」に変更し、"
      print_err "Windows のパーティションを保持したまま空き領域にインストールしてください。"
      exit 1
    fi
  fi
